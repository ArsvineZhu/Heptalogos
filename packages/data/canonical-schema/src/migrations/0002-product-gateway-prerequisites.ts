    ...
    await sql
      .raw(
        [
          ...
          `    btrim(definition_id) <> '' AND octet_length(definition_id) BETWEEN 1 AND 256 AND definition_version >= 1`,
          `  ),`,
          ...
          `  ),`,
          `  CONSTRAINT configuration_activation_definition_check CHECK (`,
          `    btrim(definition_id) <> '' AND octet_length(definition_id) BETWEEN 1 AND 256`,
          `  ),`,
          ...
          `  CONSTRAINT configuration_activation_impact_check CHECK (impact IN ('LIVE', 'RELOAD_COMPONENT', 'RESTART_COMPONENT', 'RESTART_SUBJECT', 'RESTART_HOST', 'MAINTENANCE', 'NEXT_BOOT', 'IMMUTABLE_AFTER_INIT')),`,
          `  CONSTRAINT configuration_activation_definition_scope_unique UNIQUE (definition_id, scope_key)`,
          `);`,
