...
2. Run the Mechanics Ownership and Library-First playbook before writing a generic helper.
3. Read the role's `RoleDecision` and implementation route.
4. If `ADOPTED`, use the adopted facility through the designated Heptalogos adapter/facade. Missing wrapper code means implement the wrapper, not a parallel substitute.
5. If the role is genuinely new, perform the smallest evidence step that resolves the unknown property. Do not build a miniature Heptalogos to choose a library.
6. Resolve any temporary candidate to `ADOPTED`, `DEFERRED`, or `REJECTED_FOR_ROLE` before it enters an implementation plan.
7. Research exact versions from current registry/upstream evidence at Catalog freeze/upgrade time; architecture documents define role/compatible line, not a guessed patch version.
8. Keep direct versions in the pnpm Catalog and resolved closure in the lockfile. Do not select independent local ranges.
9. Check native/transitive executables, `.node`, shared libraries, WASM, helpers, source-less packaging, SBOM, licensing, and platform closure when relevant.
10. Keep framework/library objects below stable Heptalogos contracts.
