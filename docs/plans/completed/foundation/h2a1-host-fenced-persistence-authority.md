...
**Execution amendment (2026-08-24):** PostgreSQL 18 requires `UPDATE` privilege for a direct `SELECT ... FOR SHARE`, which conflicts with the required runtime absence of fence-table `UPDATE`. The schema authority therefore installs the exact `FOR SHARE` query as the body of the owner-owned SQL `SECURITY DEFINER` function `heptalogos.lock_host_ownership_fence()`, with `search_path = pg_catalog`; the runtime role receives only `EXECUTE` on that function in addition to its declared `SELECT`/schema/database privileges. `@heptalogos/persistence` invokes the function, so the row lock still belongs to the enclosing transaction and is held through commit/rollback. Direct runtime `FOR SHARE` remains denied and is recorded as qualification evidence; no lock mode or runtime table-DML privilege is widened.
...
5. **The runtime principal never owns the product schema and cannot publish/revoke the Host token.** It receives `CONNECT`, product-schema `USAGE`, fence `SELECT`, `EXECUTE` only on the schema-authority-owned fence-lock function, and later per-table DML granted by migration/schema authority. H2A-1 does not install default DML grants for future product tables.
