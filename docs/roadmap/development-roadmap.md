...
- M5A implementation candidate `3735e343f7684fa79fcd57f01305b7d1285e411b` has deterministic unit/build/boundary evidence for the MaintenanceJournal, bootstrap-admin revocation, quiesced managed Host, existing-cluster maintenance controller, stop/exit, restart/reacquire, and fault policy. Real PostgreSQL M5A qualification is `NOT_RUN`/`BLOCKED` because the explicit `HEPTALOGOS_TEST_PG_BIN` qualification root was not available on this host; independent review and final cross-platform CI are also `NOT_RUN`.
