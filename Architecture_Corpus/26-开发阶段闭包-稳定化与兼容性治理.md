...
稳定化默认使用一个 bounded plan、一个短生命周期 branch、一个 Draft PR 和一个 final
external closure tuple：

```text
ReviewCandidate = (base_sha, head_sha)
```

Hn-S candidate 必须在 Independent Review 前完成所有 repository mutation。只有 exact
pair Independent Review `PASS`、同一 pair 的 Ubuntu/macOS/Windows manual final CI `PASS`、
merge 前 base/head 未移动并 squash merge 成功后，才发生行为候选闭包。合并后的 behavior
candidate 不可变；current truth 只能由 separate docs/evidence-only reconciliation PR
引用外部 review/CI/merge evidence 后更新。
