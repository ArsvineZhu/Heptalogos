...
Base movement alone does not invalidate Independent Review when the reviewed PR diff remains
semantically unchanged and no conflict-resolution/rebase commit is added to the PR branch.
Base movement invalidates final integration CI, which must be rerun against the current base.
