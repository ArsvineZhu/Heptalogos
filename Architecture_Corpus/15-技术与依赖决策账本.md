...
| Bootstrap lock | `proper-lockfile` 4.x | ADOPTED | REQUIRED | 仅覆盖 pre-PostgreSQL ownership window；正常 Host Authority 是 PostgreSQL lease + HostOwnershipFence |
...
```text
pre-PostgreSQL lock → proper-lockfile 4.x
normal Host authority → dedicated PostgreSQL advisory lease + HostOwnershipFence + HostOwnershipToken
```

`proper-lockfile` 不得扩展成第二套长期 ownership system。power-loss、跨平台和 source-less bootstrap 属 implementation qualification。
