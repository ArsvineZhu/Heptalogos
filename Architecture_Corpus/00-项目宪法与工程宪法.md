...
当前项目的 `CompatibilityEpoch = PRE_PRODUCTION`。版本字段仍然是 durable contract 的强制身份；但 merged commit、内部开发状态、可重新生成的 fixture 或曾经存在的开发格式本身都不构成历史兼容义务。只有明确保留的 durable state 或明确的 external consumer 才能建立 compatibility obligation；没有该义务时，当前最佳 contract 可以 canonicalize 为 V1，过时的 reader/migration/shim 必须删除并拒绝 obsolete shape。
